...
class CSVWriter
{
    public:
		...
		streampos overwriteFile(string filename) {
			return overwriteFile(filename, false, 0);
		}

		streampos overwriteFile(string filename, bool append, streampos pos) {
			ofstream file;
			if (append)
				file.open(filename.c_str(), ios::out | ios::app);
			else
			...
			file.write(this->toString().c_str(), strlen(this->toString().c_str()));
			file << endl;
